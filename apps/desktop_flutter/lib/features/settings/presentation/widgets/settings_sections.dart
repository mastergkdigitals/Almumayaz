      ...
      children: [
        _SettingsManagementPanel(
          ...
          initialEntries: const [
            ...
            _SettingsNamedEntry(id: 2, name: 'المصاريف العامة'),
            _SettingsNamedEntry(id: 3, name: 'حساب الأطراف'),
          ],
        ),
        _SettingsManagementPanel(
...
class _SettingsNamedEntry {
  ...
  _SettingsNamedEntry copyWith({String? name}) {
...
class _SettingsManagementPanel extends StatefulWidget {
  ...
  final IconData icon;
  final Color accentColor;
  final List<_SettingsNamedEntry> initialEntries;
...
class _SettingsManagementPanelState
  ...
  late List<_SettingsNamedEntry> _entries;
  int? _selectedId;
  ...
  void initState() {
    super.initState();
    _entries = List<_SettingsNamedEntry>.of(widget.initialEntries);
    _selectedParent =
  ...
  void _addEntry() {
    ...
    final nextId = _entries.fold<int>(
        ...
        1;
    setState(() {
      _entries.add(_SettingsNamedEntry(id: nextId, name: name));
      _selectedId = nextId;
    });
  ...
  void _updateEntry() {
    ...
    final index = _entries.indexWhere((entry) => entry.id == selectedId);
    if (index < 0) return;
    setState(() => _entries[index] = _entries[index].copyWith(name: name));
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              AppButton(
                ...
                minWidth: 115,
                onPressed: _selectedId == null ? null : _updateEntry,
              ),
              AppButton(
          ...
          AppDataTable(
            ...
            showShadow: false,
            columns: const [
              AppTableColumn(label: 'ت', flex: 0.45, numeric: true),
              AppTableColumn(label: 'الاسم', flex: 1.8),
              AppTableColumn(label: 'الحالة', flex: 1),
              AppTableColumn(label: 'الإجراءات', flex: 0.8),
            ],
            rows: [
...
class _UsersSecuritySettingsSectionState
  ...
  static const _roles = <_SettingsRole>[
    _SettingsRole(
      id: 1,
      name: 'مدير النظام',
      usersCount: 1,
    ...
    _SettingsRole(
      ...
      name: 'المبيعات',
      usersCount: 1,
      permissions: 'المبيعات، الزبائن، الكشوفات',
    ),
    _SettingsRole(
      id: 3,
      name: 'الحسابات',
      usersCount: 1,
      permissions: 'الصندوق، التقارير، الكشوفات',
    ),
    _SettingsRole(
      id: 4,
      name: 'عرض فقط',
      usersCount: 0,
      permissions: 'عرض بدون تعديل أو حذف',
    ),
  ];
  ...
  void _toggleUserEnabled(_SettingsUser user) {
    ...
    if (index < 0) return;
    final nextStatus = user.status == 'نشط' ? 'معطل' : 'نشط';
    setState(() {
      _users[index] = user.copyWith(status: nextStatus);
    });
    if (nextStatus == 'نشط') {
  ...
  void _toggleUserLock(_SettingsUser user) {
    final index = _users.indexWhere((entry) => entry.id == user.id);
    if (index < 0) return;
    final nextStatus = user.status == 'مقفل' ? 'نشط' : 'مقفل';
    setState(() {
      _users[index] = user.copyWith(status: nextStatus);
    });
    AppToast.showWarning(
      context,
      nextStatus == 'مقفل' ? 'تم قفل الحساب' : 'تم فتح قفل الحساب',
    );
  }
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: AppSpacing.lg),
        switch (_selectedTab) {
          'roles' => _buildRolesTemplate(),
          'security' => _buildSecurityTemplate(),
          'logs' => _buildLogsTemplate(),
          _ => _buildUsersTemplate(),
        },
      ],
    );
  }
              ...
              for (final user in _users)
                AppTableRow(
                  ...
                  cells: [
                    ...
                    Text(user.role),
                    Center(child: _userStatusBadge(user.status)),
                    Text(user.lastLogin),
                    Center(
                      child: Wrap(
                        ...
                        children: [
                          AppTableActionButton(
                            ...
                            icon: user.status == 'نشط'
                                ? Icons.person_off_outlined
                                : Icons.person_outline_rounded,
                            tooltip: user.status == 'نشط'
                                ? 'تعطيل الحساب'
                                : 'تفعيل الحساب',
                            variant: user.status == 'نشط'
                          ...
                          AppTableActionButton(
                            ...
                            icon: user.status == 'مقفل'
                                ? Icons.lock_open_rounded
                                : Icons.lock_outline_rounded,
                            tooltip: user.status == 'مقفل'
                                ? 'فتح القفل'
                                : 'قفل الحساب',
                            variant: AppButtonVariant.warning,
                            onPressed: user.username == 'admin'
                          ...
                          AppTableActionButton(
                            ...
                            onPressed: () => AppToast.showInfo(
                              context,
                              'فتح نموذج كلمة مرور ${user.fullName}',
                            ),
                          ),
                        ],
                      ),
    ...
    return SettingsTemplatePanel(
      ...
      actions: [
        AppRegularButton(
          ...
          onPressed: () {
            AppToast.showInfo(context, 'فتح نموذج دور تجريبي جديد');
          },
        ),
      ],
      child: Column(
  ...
  Widget _userStatusBadge(String status) {
    return AppStatusBadge(
      label: status,
      tone: switch (status) {
        'نشط' => AppStatusTone.success,
        'مقفل' => AppStatusTone.danger,
...
class _SettingsUser {
  ...
  _SettingsUser copyWith({String? status}) {
        ...
        children: [
          ...
          AppDropdownField<String>(
            ...
            options: const [
              ...
              AppDropdownOption(value: 'المبيعات', label: 'المبيعات'),
              AppDropdownOption(value: 'الحسابات', label: 'الحسابات'),
              AppDropdownOption(value: 'عرض فقط', label: 'عرض فقط'),
            ],
            useIntrinsicHeight: true,
            textDirection: TextDirection.rtl,
            textAlign: TextAlign.right,
